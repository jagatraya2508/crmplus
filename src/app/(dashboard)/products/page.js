...
export default function ProductsPage() {
    ...
    const [form, setForm] = useState({ name: '', sku: '', description: '', price: 0, unit: 'pcs', category: '', stock: 0 });
    ...
    function openAdd() { setEditing(null); setForm({ name: '', sku: '', description: '', price: 0, unit: 'pcs', category: '', stock: 0 }); setShowModal(true); }
    function openEdit(p) { setEditing(p); setForm({ name: p.name, sku: p.sku || '', description: p.description || '', price: parseFloat(p.price), unit: p.unit, category: p.category || '', stock: p.stock }); setShowModal(true); }
                    ...
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="modal-body">
                                ...
                                <div className="form-group"><label className="form-label">Kategori</label><input className="form-control" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} /></div>
